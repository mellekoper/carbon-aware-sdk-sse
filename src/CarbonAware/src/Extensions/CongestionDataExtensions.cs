...
internal static class CongestionDataExtensions
{
    /// <summary>
    ...
    ///         duration is longer than the granularity of the CongestionData. Take the following examples of values for 5 
    ...
    ///     </para>
    /// </remarks>
    /// <param name="data">The IEnumerable<CongestionData> being operated on.</param>
    ...
    /// <returns>An enumerable of congestion data objects, each representing a single average window.</returns>
    ...
    // public static IEnumerable<CongestionData> RollingAverage(this IEnumerable<CongestionData> data, TimeSpan windowSize = default, DateTimeOffset dataStartAt = default, DateTimeOffset dataEndAt = default, TimeSpan tickSize = default)
    // {
    ...
    //     var q = new Queue<CongestionData>();
    //     var _data = data.GetEnumerator();
    //     _data.MoveNext();
    //     CongestionData current = _data.Current;
    //     CongestionData? last = null;
    ...
    //         {
    //             yield return new CongestionData()
    //             {
    //                 Time = windowStartTime,
    //                 Duration = windowEndTime - windowStartTime,
    //                 Location = last.Location,
    //                 Difference = AverageOverPeriod(q, windowStartTime, windowEndTime)
    //             };
    //         }
    ...
    /// <summary>
    /// Finds the average difference of a continuous, chronological set of CongestionData objects for a given period. 
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         This method takes into account fractional data points and weights the average accordingly.
    ///         Take the following (HH:MM, difference) hour-long data points for example: (01:00, 30), (02:00, 100), (03:00, 50)
    ///     </para>
    ///     <para>
    ...
    {
        CongestionData? current = null;
        CongestionData? next = null;
        var _data = data.GetEnumerator();
    ...
    private static bool IsContinuousChronological(CongestionData current, CongestionData previous, bool reverse)
    {
        if (reverse){
